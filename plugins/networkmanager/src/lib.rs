//! NetworkManager plugin - provides network connectivity management.
//!
//! This is a dynamic plugin (.so) loaded by waft-overview at runtime.
//! Manages WiFi, Ethernet, and VPN connections through NetworkManager.
...
use waft_plugin_api::{OverviewPlugin, PluginId, PluginResources, WidgetRegistrar};
use anyhow::Result;
...
impl Default for NetworkManagerPlugin {
    fn default() -> Self {
...
impl OverviewPlugin for NetworkManagerPlugin {
    ...
    async fn init(&mut self, resources: &PluginResources) -> Result<()> {
        ...
        // Create NetworkManager instance on a dedicated thread with block_on.
        // nmrs proxy types use PropertiesCache which spawns internal tokio tasks.
        // With enter() those tasks run on host worker threads where the plugin's
        // tokio has no context, causing panics. block_on keeps everything on one
        // thread with full runtime context.
        let nm: NetworkManager = runtime_bridge::spawn_on_tokio(async {
            NetworkManager::new().await
                .map_err(|e| anyhow::anyhow!("Failed to create NetworkManager: {e}"))
        }).await?;

        let nm_for_devices = nm.clone();
        let devices_result: Result<Vec<dbus::DeviceInfo>> = runtime_bridge::spawn_on_tokio(async move {
            dbus::get_all_devices_nmrs(&nm_for_devices).await
        }).await;
    ...
    async fn create_elements(
        ...
        let dbus = self.dbus.as_ref().expect("dbus not initialized").clone();
        ...
        if !vpn_connections.is_empty() {
            ...
            if let Ok(active_vpns) = dbus::get_active_vpn_connections(&dbus).await {
...
impl NetworkManagerPlugin {
            ...
            if let Err(e) =
                dbus::subscribe_vpn_state_changed(dbus.clone(), move |_path, state_code| {
                    ...
                    glib::spawn_future_local(async move {
                        ...
                        let vpn_connections = match crate::runtime_bridge::spawn_on_tokio(
                        ...
                        let active_vpns = match crate::runtime_bridge::spawn_on_tokio(
    ...
    fn subscribe_device_signals(&self) {
        ...
        glib::spawn_future_local(async move {
            if let Err(e) = dbus::subscribe_device_added(dbus_for_add.clone(), move |device_path| {
                ...
                glib::spawn_future_local(async move {
                    ...
                    let device_info = match crate::runtime_bridge::spawn_on_tokio(
        ...
        let dbus_for_state = self.dbus.as_ref().expect("dbus not initialized").clone();
