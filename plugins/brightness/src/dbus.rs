...
/// Run a one-shot command on a background thread.
///
...
async fn run_command(
    ...
    rx.recv_async().await.map_err(|e| anyhow!("Command task cancelled: {}", e))?
}

/// Check if brightnessctl CLI is available.
pub async fn is_brightnessctl_available(tokio_handle: &tokio::runtime::Handle) -> bool {
    run_command("which", &["brightnessctl"], tokio_handle)
        .await
...
pub async fn is_ddcutil_available(tokio_handle: &tokio::runtime::Handle) -> bool {
    run_command("which", &["ddcutil"], tokio_handle)
        .await
        .map(|o| o.status.success())
        .unwrap_or(false)
}
...
pub async fn discover_backlight_devices(
    tokio_handle: &tokio::runtime::Handle,
) -> Result<Vec<DiscoveredDisplay>> {
    let output = run_command("brightnessctl", &["-l", "-m", "-c", "backlight"], tokio_handle).await?;
...
pub async fn discover_ddc_monitors(
    tokio_handle: &tokio::runtime::Handle,
) -> Result<Vec<DiscoveredDisplay>> {
    let output = run_command("ddcutil", &["detect", "--brief"], tokio_handle).await?;
    ...
    for line in stdout.lines() {
        ...
        if trimmed.starts_with("Display ") {
            ...
            if let (Some(display_num), Some(model)) = (current_display.take(), current_model.take())
                && let Ok(brightness) = get_ddc_brightness(display_num, tokio_handle).await {
                    displays.push(DiscoveredDisplay {
...
async fn get_ddc_brightness(display_num: u32, tokio_handle: &tokio::runtime::Handle) -> Result<f64> {
...
pub async fn set_brightness(
    ...
    if let Some(device_name) = device_id.strip_prefix("backlight:") {
        set_backlight_brightness(device_name, value, tokio_handle).await
    } else if let Some(display_num) = device_id.strip_prefix("ddc:") {
        let num: u32 = display_num.parse()?;
        set_ddc_brightness(num, value, tokio_handle).await
    } else {
...
async fn set_backlight_brightness(
    device_name: &str,
    value: f64,
    tokio_handle: &tokio::runtime::Handle,
) -> Result<()> {
