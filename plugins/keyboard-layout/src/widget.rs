...
/// Run an async task on a background thread with tokio context and return its result.
///
/// Uses std::thread + handle.block_on() to avoid running on the host's tokio
/// worker threads (where the plugin's tokio context isn't available in cdylib).
/// The handle must be stored from init() since Handle::current() won't work
/// on the GTK main thread after the enter() guard expires.
    ...
    T: Send + 'static,
{
    ...
    match rx.recv_async().await {
        Ok(val) => val,
        Err(e) => {
            error!("[runtime] background task was cancelled or panicked: {e}");
            panic!("background task was cancelled or panicked: {e}");
        }
    }
}
...
impl KeyboardLayoutWidget {
    ...
    fn query_and_update_label(&self) {
        ...
        glib::spawn_future_local(async move {
            ...
            let result: anyhow::Result<String> = spawn_on_tokio(handle, async move {
