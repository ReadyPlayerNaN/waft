...
impl HyprlandBackend {
    ...
    async fn get_configured_layouts() -> Result<Vec<String>> {
        let output = run_hyprctl(&["getoption", "input:kb_layout", "-j"]).await?;

        if !output.status.success() {
            // Fallback: try without -j flag and parse manually
            let output = run_hyprctl(&["getoption", "input:kb_layout"]).await?;
    ...
    async fn query_active_keymap(&self) -> Result<String> {
        let output = run_hyprctl(&["devices", "-j"]).await?;
...
impl KeyboardLayoutBackend for HyprlandBackend {
    ...
    async fn switch_next(&self) -> Result<()> {
        let output = run_hyprctl(&["switchxkblayout", "all", "next"]).await?;
    ...
    async fn switch_prev(&self) -> Result<()> {
        let output = run_hyprctl(&["switchxkblayout", "all", "prev"]).await?;
    ...
    fn subscribe(&self, sender: Sender<LayoutEvent>) {
        ...
        // Use std::thread + std::os::unix::net for the socket to avoid tokio IO reactor
        std::thread::spawn(move || {
            use std::io::BufRead;
            ...
            for line in reader.lines() {
                ...
                if let Some((_keyboard, layout_name)) = line
