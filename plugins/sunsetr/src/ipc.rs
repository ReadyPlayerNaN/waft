...
/// Spawn following task on a background thread.
///
/// Uses std::process::Command + std::thread to avoid needing tokio's IO reactor
/// (which is in the host's copy of tokio, not the plugin's cdylib copy).
pub fn spawn_following_with_handle(
    sender: Sender<SunsetrIpcEvents>,
...
async fn run_sunsetr(args: &[&str]) -> anyhow::Result<(i32, String, String)> {
    ...
    std::thread::spawn(move || {
        let result = std::process::Command::new("sunsetr")
    ...
    let out = rx
        .recv_async()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to receive response from sunsetr thread: {e}"))?
...
async fn refresh_after_start() -> anyhow::Result<Status> {
    ...
    for attempt in 0..20 {
        ...
        // Use blocking sleep since this runs in handle.spawn() on host worker threads
        // where the plugin's tokio timer is not available
