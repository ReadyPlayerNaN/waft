...
impl NiriBackend {
    ...
    pub async fn new() -> Option<Self> {
        ...
        let output = Command::new("niri").arg("--version").output().await.ok()?;
    ...
    /// Execute a layout switch action.
...
impl KeyboardLayoutBackend for NiriBackend {
    ...
    fn subscribe(&self, sender: Sender<LayoutEvent>) {
        ...
        tokio::spawn(async move {
            log::info!("[keyboard-layout:niri] Tokio task started, spawning niri event-stream");
            ...
            // so we can use them when we receive KeyboardLayoutSwitched events
            ...
            loop {
                match lines.next_line().await {
                        ...
                        match serde_json::from_str::<NiriEvent>(line) {
                            Ok(event) => {
                                ...
                                if let Some(payload) = event.keyboard_layouts_changed {
                                    ...
                                    // Cache the layout names for future switch events
                                ...
                                // Handle layout switch (just index, use cached names)
                                    ...
                                    if !cached_layouts.is_empty() {
                                        let available: Vec<String> = cached_layouts
                                            ...
                                            .map(|n| extract_abbreviation(n))
                                            .collect();
                                        let current_index = payload.idx.min(available.len().saturating_sub(1));
                                        let current = available
                                            .get(current_index)
                                            .cloned()
                                            .unwrap_or_else(|| "??".to_string());
                                        ...
                                        log::warn!("[keyboard-layout:niri] Got switch event but no cached layouts");
                                    }
                                }
                    ...
                    Ok(None) => {
                        log::warn!("[keyboard-layout:niri] Event stream EOF - niri process likely exited");
                        break;
                    }
                    Err(e) => {
