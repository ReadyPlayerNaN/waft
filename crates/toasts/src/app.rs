...
pub async fn setup(position: ToastPosition) -> Result<adw::Application, Box<dyn std::error::Error>> {
    ...
    tokio::spawn(daemon_connection_task(event_tx, client_handle_clone));
...
fn handle_event(event: ToastEvent, manager: &ToastManager, window: &ToastWindow) {
    match event {
        ToastEvent::Connected => {
            log::info!("[toasts] connected to daemon");
        }
        ToastEvent::Disconnected => {
