...
use super::agenda_details::{AgendaDetailsComponent, AgendaDetailsProps};
...
pub struct AgendaCard {
    pub root: gtk::Box,
    menu_chevron: Option<Rc<MenuChevronWidget>>,
...
impl AgendaCard {
    pub fn new(
        ...
        if has_details(event) {
            ...
            let details_props = AgendaDetailsProps {
                location: event.location.clone(),
                attendees: event
                    .attendees
                    .iter()
                    .map(|a| (a.email.clone(), a.name.clone(), a.status))
                    .collect(),
                description: event.description.clone(),
            };
            let details = AgendaDetailsComponent::build(&details_props);
            revealer.set_child(Some(&details.widget()));
    ...
    pub fn set_expanded(&self, expanded: bool) {
        if let Some(ref chevron) = self.menu_chevron {
            chevron.update(&MenuChevronProps { expanded });
        }
